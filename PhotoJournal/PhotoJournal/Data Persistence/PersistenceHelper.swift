...
class PersistenceHelper {
  ...
  // array of events
  private var events = [PhotoObject]()
  ...
  // for re-ordering
  public func reorderEvents(events: [PhotoObject]) {
    self.events = events
    try? save()
  }
  ...
  public func create(event: PhotoObject) throws {
    // step 2.
  ...
  public func loadPhotos() throws -> [PhotoObject] {
    ...
    if FileManager.default.fileExists(atPath: url.path) {
      if let data = FileManager.default.contents(atPath: url.path) {
        do {
          events = try PropertyListDecoder().decode([PhotoObject].self, from: data)
        } catch {
